...
class Topic(object):
    def __init__(self, index=0):
        ...
        # self.word_distribution = defaultdict(int)
        self.first_observed_time = 0
        # self.word_count = 0
        self.alpha = None
        self.doc_nums = 0
        self.count = 1
    ...
    def add_document(self, doc):
        ...
        # self.word_freq = self.word_distribution / sum(self.word_distribution)
    ...
    def add_doc_dict(self, doc):
        ...
        for word in doc.word_distribution:
            self.word_distribution[word] += doc.word_distribution[word]
            # self.word_freq[word] = self.word_distribution[word] / self.word_count
    '''
    def update_sumkernel(self, t):
        new_sum = []
        new_time = []
        for i in range(len(self.time_seq)):
            if self.time_seq[i] > t:
                new_sum.append(self.sum_kernel[i])
                new_time.append(self.time_seq[i])
        self.sum_kernel = new_sum
        self.time_seq = new_time
        if self.time_seq:
            return self.time_seq[0]
        else:
            return 100000000000

    def __copy__(self):
        new_t = Topic(self.index)
        new_t.alpha = copy(self.alpha)
        new_t.first_observed_time = self.first_observed_time
        new_t.doc_nums = self.doc_nums
        new_t.count = self.count
        new_t.sum_kernel = self.sum_kernel.copy()
        return new_t

    def go_back(self, doc):
        self.word_count -= doc.word_count
        for word in doc.word_distribution:
            self.word_distribution[word] -= doc.word_distribution[word]
            self.word_freq[word] = self.word_distribution[word] / self.word_count
...
class Particle(object):
    ...
    def reset(self):
        self.log_update_prob = 0
        self.clusters = OrderedDict()  # can be store in the process for efficient memory implementation, key = cluster_index, value = particle object
        self.docs2cluster_ID = []  # the element is the cluster index of a sequence of document ordered by the index of document
        self.active_clusters = {}  # dict key = cluster_index, value = list of timestamps in specific cluster (queue)
        self.cluster_num_by_now = 0

        self.docs2topic_ID = []
        self.cluster2topic_ID = OrderedDict()
        self.alpha = OrderedDict()  # key = topic_index, value = alphas for a specific topic
    ...
    def copy(self, particle):
        ...
        self.docs2cluster_ID = particle.docs2cluster_ID
...
def perplexity(particle, test_set, idx, theta0):
    log_llh = []
    total_count = 0
    # particle = inferer.particles[0]
    for doc in test_set:
        doc = parse_newsitem_2_doc(news_item=doc, vocabulary_size=100, words_idx=idx, time_unit=3600)
        topic_likelihood = []
        for t in particle.topics:
            topic_word_distribution = particle.topic_word_distribution[t]
            topic_word_count = particle.topic_word_count[t]
            llh = log_dirichlet_multinomial_distribution_dict(topic_word_distribution,
                                                                        doc.word_distribution,
                                                                        topic_word_count,
                                                                        doc.word_count,
                                                                        theta0)
            topic_likelihood.append(llh)
        total_count += doc.word_count
        log_llh.append(max(topic_likelihood))
        # return np.exp(-sum(log_llh)/len(test_set))
    # print(log_llh)
    return sum(log_llh)/total_count


def perplexity_dhp(particle, test_set, idx, theta0):
    log_llh = []
    total_count = 0
    for doc in test_set:
        doc = dhp.parse_newsitem_2_doc(news_item=doc, vocabulary_size=10000, words_idx=idx)
        topic_likelihood = []
        for t in particle.clusters:
            topic_word_distribution = particle.clusters[t].word_distribution
            topic_word_count = particle.clusters[t].word_count
            llh = log_dirichlet_multinomial_distribution(topic_word_distribution,
                                                                        doc.word_distribution,
                                                                        topic_word_count,
                                                                        doc.word_count, 0,
                                                                        theta0)
